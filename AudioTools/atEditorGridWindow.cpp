...
namespace editor {
	...
	void GridWindow::SetupControl(ax::Window* window)
	{
		...
		window->event.OnMouseLeftDown
			= ax::WFunc<ax::Point>([gwin, window, m_down_fct](const ax::Point& pos) {
				  ...
				  if (cmd_down) {
					  ...
					  gwin->PushEvent(1234, new ax::Event::SimpleMsg<ax::Window*>(window));
				  }
				  else {
		...
		window->event.OnMouseLeftDragging = ax::WFunc<ax::Point>([window, m_drag_fct](const ax::Point& pos) {
		...
		window->event.OnMouseLeftUp = ax::WFunc<ax::Point>([window, m_up_fct](const ax::Point& pos) {
		...
		window->event.OnMouseRightDown = ax::WFunc<ax::Point>([window, m_right_down](const ax::Point& pos) {
			ax::Print("RIGHT DOWN");
